package de.nulldrei.oop.ex5.gui.baelle;
   

import de.nulldrei.oop.ex5.business.baelle.BaelleModel;
import de.nulldrei.oop.ex5.business.baelle.Ball;
import de.nulldrei.oop.ex5.ownUtil.MeldungsfensterAnzeiger;
import de.nulldrei.oop.ex5.ownUtil.Observable;
import de.nulldrei.oop.ex5.ownUtil.Observer;
import javafx.event.*;
...
public class BaelleView implements Observable {
   ...
   private void initListener() {
	   cmbBxEinkaufdatum.setOnAction(e -> {
		   zeigeBallAn();
	   });
	   btnAenderungAufnehmen.setOnAction(e -> {
	   ...
	   cmbBxEinkaufdatum.getItems().clear();
		for(int i = 0; i < baelleModel.holeBaelle().getAnzahlSportartikel(); i++) {
			cmbBxEinkaufdatum.getItems().add(
				baelleModel.holeBaelle().getSportartikel(i).getEinkaufsdatum() + "");
		}  
	}
